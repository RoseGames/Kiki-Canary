 ...
 */
    ...
    run: async (client, message, args, Discord) => {
        ...
        const SuccessWarnEmbed = new MessageEmbed()
        .setColor('#FC94AF')
       .setTitle(`Successful!`)
       .setDescription(`Warned ${user} successfully & logged it to `)
       .setTimestamp()
        if (!reason) return message.channel.send({embeds: [WaitWarnEmbed]});
        if (user && reason) {
            await user.kick({
                reason: reason,
            }).then(() => {
                message.channel.send({embeds: [SuccessWarnEmbed] });
            })
        } else {
